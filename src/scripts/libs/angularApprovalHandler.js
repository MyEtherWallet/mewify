...
angularApprovalHandler.prototype.setScope = function(scope) {
    ...
    this.allDivs = ['showInitDiv', 'showConfirmTxDiv', 'showTxResultDiv','showConfirmSignDiv','showSignResultDiv'];
}
...
angularApprovalHandler.prototype.showSignConfirm = function(tx, callback) {
    var _this = this;
    if (_this.scope.isTxConfirmActive) {
        callback({ error: true, msg: 'Signing denied - waiting for approval on another tx', data: false });
        return;
    }
    _this.scope.isTxConfirmActive = true;
    this.showDiv('showConfirmSignDiv');
    tx.responseSent = false;
    tx.approve = function() {
        nwGui.setAlwaysOnTop(false);
        _this.showDiv('showSignResultDiv');
        _this.scope.isTxConfirmActive = false;
        tx.responseSent = true;
        if (!_this.scope.$$phase) _this.scope.$apply();
        callback({ error: false, msg: '', data: true });
    ...
    tx.deny = function() {
        ...
        tx.responseSent = true;
        if (!_this.scope.$$phase) _this.scope.$apply();
        callback({ error: true, msg: 'Signing denied', data: false });
    }
    tx.callback = function(data) {
        tx.error = data.error;
        tx.msg = data.msg;
